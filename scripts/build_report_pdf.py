...
def build():
    ...
    s += UL([
        ...
        "宅建士66件・2級建築施工管理技士22件などはすべて優遇条件であり、"
    ...
    s.append(P("求人票の「仕事内容」「必要な経験等」「特記事項」に、その業務・スキルの記述があった求人の割合。", "note"))
    hdr = ["業務・スキル", "営業\n183", "施工管理\n52", "施工技能\n42", "設備工事\n22",
           "建材営業\n17", "メンテ\n17", "設計CAD\n14"]
    rows = [hdr] + [
        ...
        ["企業側の未経験者育成の明記（研修・OJT）", "39%", "25%", "62%", "45%", "65%", "71%", "21%"],
        ["企業側の資格取得支援の明記", "2%", "6%", "5%", "9%", "0%", "0%", "0%"],
    ]
    s.append(table(rows, [175, 47, 47, 47, 47, 47, 47, 48], fs="small", pad=3.4))
    s.append(P("読み取れること", "h2"))
    s += UL([
        "<b>7職種すべてに共通するのは「対話」と「PC入力」の2つだけで、技術内容は職種ごとに完全に分かれる。</b>"
        "共通コア科目にできるのはこの2領域と、ほぼ全職種で必須の普通自動車運転免許に限られる。",
        "<b>企業側が「未経験者を育てる」と明記しているのは技能系62%・メンテ71%・建材営業65%。</b>"
        "一方で施工管理25%・設計21%は低い。訓練の役割が「企業の育成工数を減らす下地づくり」か"
        "「肩代わり」かは職種で変わる。",
        "<b>資格取得支援を明記する求人は全職種で1割未満。</b>"
    ...
    rows += [
        ["住宅・リフォーム営業\n183件",
         "普通自動車運転免許149／宅地建物取引士10／\n二級建築士9／ＦＰ技能士9",
         "宅地建物取引士66／二級建築士17／一級建築士13／\n1・2級建築施工管理技士7／木造建築士7"],
        ["施工管理・現場監督\n52件",
         "普通自動車運転免許34／2級建築施工管理技士4／\n二級建築士4／一級建築士4",
         "2級建築施工管理技士22／1級建築施工管理技士20／\n二級建築士19／一級建築士9"],
        ["施工技能職\n42件", "普通自動車運転免許23",
         "車両系建設機械（整地等）運転技能者4／中型自動車免許3／\n第二種電気工事士2"],
        ["設備工事\n22件", "普通自動車運転免許18／第二種電気工事士2／\n給水装置工事責任技術者等1",
